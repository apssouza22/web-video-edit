...
// MediaBunny library type definitions
declare module "https://cdn.jsdelivr.net/npm/mediabunny@1.0.2/+esm" {
  ...
  interface AudioBufferSourceConfig {
    codec: string;
    bitrate: number;
  }
  ...
  export class CanvasSource {
    constructor(canvas: OffscreenCanvas, config: CanvasSourceConfig);
    add(timestamp: number, duration: number): Promise<void>;
    close(): void;
  }

  export class AudioBufferSource {
    constructor(config: AudioBufferSourceConfig);
  ...
  export class Output {
    ...
    addVideoTrack(source: CanvasSource, config?: TrackConfig): void;
    addAudioTrack(source: AudioBufferSource): void;
    start(): Promise<void>;
  ...
  export const QUALITY_HIGH: number;
  ...
  export function getFirstEncodableAudioCodec(
    codecs: string[],
